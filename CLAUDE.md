...
- **No environment-specific defaults in `app/config.py`.** Database URLs, secrets, hostnames, and cookie settings are required and have no fallback value. Missing config MUST raise at startup. An app that silently starts against a default is worse than an app that refuses to start, because the failure moves from my terminal to my mother's data.
